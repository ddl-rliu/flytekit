...
class FileDownloadConfig:
    ...
    @staticmethod
    def check_and_convert_to_file_extension(item: typing.Union[typing.Type, str]) -> Optional[str]:
        if get_origin(item) is Annotated:
            for arg in get_args(item):
                if isinstance(arg, FileDownloadConfig):
                    return arg.file_extension
        return None

    @staticmethod
    def check_and_convert_to_enable_legacy_filename(item: typing.Union[typing.Type, str]) -> Optional[str]:
        if get_origin(item) is Annotated:
            for arg in get_args(item):
                if isinstance(arg, FileDownloadConfig):
                    return arg.enable_legacy_filename
        return None
